    ...
    fn convert_expand(
        ...
        let join_sql = match direction {
            RelationshipDirection::Outgoing => format!(
                ...
                input_sql, src_alias, rel_type, rel_alias, src_alias, rel_alias, rel_mapping.source_id_field,
                target_variable, tgt_alias, rel_alias, rel_mapping.target_id_field, tgt_alias
            ),
        ...
        if !properties.is_empty() {
            let rel_filters = properties.iter()
                .map(|(k, v)| Ok(format!("{}.{} = {}", rel_alias, k, self.property_value_to_sql(v)?)))
                .collect::<Result<Vec<_>>>()?
                .join(" AND ");
