...
MutableTableMapping = MutableMapping[str, pa.Table]
...
class KnowledgeGraph:
    ...
    def __init__(self, config: GraphConfig, datasets: TableMapping) -> None:
        ...
        normalized = {name: _ensure_table(name, table) for name, table in datasets.items()}
    ...
    def run(
        ...
        *,
        ...
        if datasets:
            sources.update({name: _ensure_table(name, table) for name, table in datasets.items()})
